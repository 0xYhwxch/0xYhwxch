<h1 align="center">
   Hi there
  <img src="https://raw.githubusercontent.com/ABSphreak/ABSphreak/master/gifs/Hi.gif" width="28px">
</h1>

<h3 align="center"> 🛡️ Aspiring Smart Contract Engineer | 💻 CS Graduate | 🔍 Blockchain Security Enthusiast </h3>

---

## 🧠 Who Am I?

🧪 **Aspiring Web3 Security-Focused Developer Building and testing Ethereum smart contracts**.

I enjoy diving deep into **EVM internals**, uncovering subtle vulnerabilities, and understanding how exploits really happen.

Currently diving deep into **advanced Foundry, smart contract auditing, and EVM internals including Assembly, Yul, Huff, and Formal Verification** ,while also started learning **Rust 🦀** to master **memory-safe**.  

🐛 **Bug hunter mindset**.

---

## 🤖 Tech Stack ### 

### 🔗 Programming Languages
![Solidity](https://img.shields.io/badge/Solidity-363636?style=for-the-badge&logo=solidity&logoColor=00ffff)
![Rust](https://img.shields.io/badge/Rust-000000?style=for-the-badge&logo=rust&logoColor=ff0080)
![JavaScript](https://img.shields.io/badge/JavaScript-111111?style=for-the-badge&logo=javascript&logoColor=00ffea)
![Python](https://img.shields.io/badge/Python-000000?style=for-the-badge&logo=python&logoColor=ffd43b)

### 🌐 Web & Frontend 
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white) ![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white) ![Bootstrap](https://img.shields.io/badge/Bootstrap-7952B3?style=for-the-badge&logo=bootstrap&logoColor=white)

### 🛠 Frameworks & Libraries 
![Foundry](https://img.shields.io/badge/Foundry-7C3AED?style=for-the-badge&logoColor=white) 

### 🧰 Tools & Platforms 
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white) ![VS Code](https://img.shields.io/badge/VS%20Code-007ACC?style=for-the-badge&logo=visual-studio-code&logoColor=white)

---

## 🧩 Areas of Interest 

- 🛡️ **Smart Contract Auditing**
- 🧪 **Foundry Testing** — Fuzzing & Invariants 
- ⚙️ **EVM Internals & Gas Optimization**
- ⚡ **Assembly & Formal Verification** — **EVM • Yul • Huff**
- 🦀 **Rust**

---

## 👾 Currently Sharpening

- 🦀 **Advanced Rust**
  - Ownership & lifetimes
  - Unsafe Rust, marcos & memory models    
- 🔍 **Smart Contract Security**
  - Reentrancy & logic bugs
  - Smart Contract Security    
- 🧪 **Foundry Fuzz Testing & Invariant testing**
- ⚡ **Assembly & Formal Verification**
  -  EVM Assembly → **raw opcodes, max control, gas mastery**
  -  Yul → **Solidity’s low-level backbone**
  -  Huff → **extreme EVM optimization at opcode level**
         
---

## 📡 Let’s Connect

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/0xniyat/)

---

## ⚡ Fun Fact 

I enjoy exploring **Web3**, building **Secure Smart Contracts**, and dissecting **Real-World hacks to understand how protocols fail** 🔍🔥
